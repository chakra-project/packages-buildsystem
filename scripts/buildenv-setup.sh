...
{
	newline
	title2 "Creating Chroot: ${REPO}-${ARCH}"
			...
			case $option in
				d* ) 
					...
					status_start "deleting ${REPO}-${ARCH}  "
						...
						progress &
						sudo rm -rf ${REPO}-${ARCH} -v &>/dev/null
						[ -f "$PROGRESSBAR" ] && { rm -f "$PROGRESSBAR"; }
						sleep 1
...
{
	newline
	title2 "Preconfiguring buildsystem"
...
{
	newline
	title2 "Configuring buildsystem for: ${REPO}-${ARCH}"
	...
	status_start "finishing..."
		# set icon for basedir
		touch $BASEPATH/.directory &>/dev/null
		echo "[Desktop Entry]" >> $BASEPATH/.directory
		echo "Icon=start-here-kde" >> $BASEPATH/.directory
